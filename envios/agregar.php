...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    $guia=(is_string($_POST['no_guia'] ?? null) ? trim($_POST['no_guia']) : '');
    ...
    elseif ($costo!=='' && !(preg_match('/^\d{1,8}(\.\d{1,2})?$/D', $costo) === 1)) {
        ...
        elseif (
            ...
            pg_query($conn,'BEGIN');
            $result=@pg_query_params($conn,'INSERT INTO Envio (No_guia,Fecha,Hora,Costo_total,Destinatario,Fecha_entrega,ID_estado,ID_cabecera,ID_tienda) VALUES ($1,$2,$3,$4,$5,NULL,1,$6,$7)',[$guia,$fecha,$hora,$costo,$destinatario,$cabecera,$tienda]);
            if ($result) {
                pg_query($conn,'LOCK TABLE Seguimiento IN EXCLUSIVE MODE');
                $result=@pg_query_params($conn,'INSERT INTO Seguimiento (ID_seguimiento,Observacion,Hora,Fecha,No_guia,ID_estado,ID_usuario) SELECT COALESCE(MAX(ID_seguimiento),0)+1,$1,$2,$3,$4,1,$5 FROM Seguimiento',['Orden registrada',$hora,$fecha,$guia,$_SESSION['id']]);
            }
            $ok=(bool)$result;
...
<form method="post">
    ...
    <label for="no_guia">Número de guía</label><input type="text" id="no_guia" name="no_guia" maxlength="20" value="<?= htmlspecialchars(trim((string) ($guia))) ?>"  required>
    ...
    <p class="ayuda">Vacío: se usa el costo de envío más manejo de la ruta. El envío inicia en Orden nueva.</p>
