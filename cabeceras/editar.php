...
$result = pg_query_params($conn, 'SELECT id_cabecera, costo_envio, costo_manejo, id_origen, id_destino FROM Cabeceras WHERE id_cabecera=$1', [$id]);
$fila = pg_fetch_assoc($result);
if (!$fila) { http_response_code(404); exit('El registro no existe.'); }
$id_cabecera = trim((string)$fila['id_cabecera']);
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif (
        ...
        $result = @pg_query_params($conn, 'UPDATE Cabeceras SET costo_envio=$1, costo_manejo=$2, id_origen=$3, id_destino=$4 WHERE id_cabecera=$5', [$costo_envio, $costo_manejo, $id_origen, $id_destino, $id]);
        if ($result) {
...
<form method="post">
    ...
    <input id="id_cabecera" name="id_cabecera" type="number" min="1" max="2147483647" step="1" readonly required value="<?= htmlspecialchars(trim((string) ($id_cabecera))) ?>">
