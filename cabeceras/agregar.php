<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        $id_cabecera = trim($_POST["id_cabecera"]);
        ...
        if ($id_cabecera < 1 || !is_numeric($costo_envio) || !is_numeric($costo_manejo) || $costo_envio < 0 || $costo_manejo < 0) {
            $mensaje = "Revisa los campos del formulario.";
        } else {
            $query = "INSERT INTO Cabeceras (id_cabecera, costo_envio, costo_manejo, id_origen, id_destino) VALUES ($1, $2, $3, $4, $5)";
            $result = @pg_query_params($conn, $query, [$id_cabecera, $costo_envio, $costo_manejo, $id_origen, $id_destino]);
...
<head>
    ...
    <title>Agregar ruta - Courier</title>
...
<?php
    ...
    echo '    <input id="id_cabecera" name="id_cabecera" type="number" min="1" max="2147483647" step="1" required value="' . $id_cabecera . '">';
