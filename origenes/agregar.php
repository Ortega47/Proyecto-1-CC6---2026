<?php
    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        ...
        if ($id_origen < 1 || $ciudad == "" || ($cobertura != "SI" && $cobertura != "NO")) {
            $mensaje = "Revisa los campos del formulario.";
        } else {
            $query = "INSERT INTO Origen (id_origen, ciudad, cobertura) VALUES ($1, $2, $3)";
            $result = @pg_query_params($conn, $query, [$id_origen, $ciudad, $cobertura]);
...
<head>
    ...
    <title>Agregar origen - Courier</title>
...
<?php
    ...
    echo '    <label for="id_origen">ID del origen</label>';
    echo '    <input id="id_origen" name="id_origen" type="number" min="1" max="2147483647" step="1" required value="' . $id_origen . '">';
